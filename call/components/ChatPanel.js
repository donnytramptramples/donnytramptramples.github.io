function ChatPanel({ messages, onSendMessage, onClose, isReady }) {
  try {
        ...
        <div className="p-4 border-t border-gray-700">
          <div className="flex gap-2">
            <input
              ...
              disabled={!isReady}
              className="flex-1 bg-[var(--dark-surface)] border border-gray-600 rounded px-3 py-2 text-sm text-white focus:outline-none focus:border-[var(--primary-color)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              style={{ caretColor: 'white' }}
            />
            <button 
              onClick={handleSend} 
              disabled={!isReady}
              className="w-10 h-10 bg-[var(--primary-color)] hover:bg-opacity-80 rounded flex items-center justify-center transition-colors disabled:opacity-50"
            >
