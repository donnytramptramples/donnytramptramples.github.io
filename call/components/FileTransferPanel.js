function FileTransferPanel({ fileTransfers, onSendFile, onClose, isReady }) {
  try {
    ...
    return (
      <div className="h-full flex flex-col glass-effect" data-name="file-transfer-panel" data-file="components/FileTransferPanel.js">
        ...
        <div className="p-4">
          <div
            onDragOver={isReady ? handleDragOver : null}
            onDragLeave={isReady ? handleDragLeave : null}
            onDrop={isReady ? handleDrop : null}
            onClick={isReady ? () => fileInputRef.current?.click() : null}
            className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
              !isReady ? 'border-gray-700 opacity-50 cursor-not-allowed' :
              dragging ? 'border-[var(--primary-color)] bg-[var(--primary-color)] bg-opacity-10 cursor-pointer' : 
