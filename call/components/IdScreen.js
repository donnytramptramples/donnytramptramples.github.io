  ...
  try {
    ...
    const handleCall = async () => {
      ...
      try {
        ...
        onCallStart(result, null, targetId);
      } catch (error) {
        console.error('Call error:', error);
        if (error.message === 'P2P_TIMEOUT') {
          showNotification('P2P connection failed, switching to Jitsi...', 'info');
          const jitsiCall = startJitsiCall(peerId, targetId);
          onCallStart('jitsi', jitsiCall.roomName, targetId);
        } else {
