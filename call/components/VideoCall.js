function VideoCall({ call, peerId, remotePeerId, messages, setMessages, fileTransfers, setFileTransfers, onHangup, onJitsiFallback }) {
  ...
  // ✅ Force refresh local preview (solves "camera changed but preview doesn't")
  const refreshLocalPreview = React.useCallback(() => {
    if (!localVideoRef.current || !call?.localStream) return;
    try {
  ...
  React.useEffect(() => {
    ...
    if (localVideoRef.current && call.localStream) {
      ...
      // ✅ Expose globals so SettingsPanel can switch devices correctly
      window.localStream = call.localStream;
      window.localVideo = localVideoRef.current;
      window.refreshLocalPreview = refreshLocalPreview;

      // Optional: refresh if tracks are added/removed
      try {
        call.localStream.onaddtrack = refreshLocalPreview;
        call.localStream.onremovetrack = refreshLocalPreview;
      } catch (_) {}
    }
    ...
    // ✅ Prevent duplicate data channels:
    // Only one side creates it. Other side waits for ondatachannel.
    const a = String(peerId || "");
    const b = String(remotePeerId || "");
    const isInitiator = a && b ? a < b : true;
    ...
    if (isInitiator) {
      try {
        ...
        console.log("Data channel creation failed, will rely on ondatachannel:", e);
      }
    } else {
      console.log("Waiting for initiator to create data channel...");
    }
    ...
    return () => {
      try {
        if (call?.localStream) {
          call.localStream.onaddtrack = null;
          call.localStream.onremovetrack = null;
        }
      } catch (_) {}
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [call, peerId, remotePeerId, refreshLocalPreview]);
  ...
  const toggleScreenShare = async () => {
    try {
      if (!isScreenSharing) {
        ...
        const sender = call.call.peerConnection.getSenders().find((s) => s.track && s.track.kind === "video");
        if (sender) {
  ...
  const stopScreenShare = async () => {
    ...
    // ✅ Restore to CURRENT camera track (not original), so camera switching works
    ...
    // ✅ Restore preview to the call.localStream (which contains current camera)
    refreshLocalPreview();
  ...
  const sendMessage = (text) => {
    ...
    // ✅ functional update prevents stale state loss
  ...
  const handleDataChannelMessage = (event) => {
    ...
    for (const fileId in fileChunksRef.current) {
      ...
      fileData.receivedSize += event.data.byteLength || 0;

      const progress = Math.min(100, Math.round((fileData.receivedSize / fileData.size) * 100));
      setFileTransfers((prev) =>
        prev.map((t) => (t.id == fileId ? { ...t, progress } : t))
      );
    }
  };
