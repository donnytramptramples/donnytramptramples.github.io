function VideoCall({ call, messages, setMessages, fileTransfers, setFileTransfers, onHangup, onJitsiFallback }) {
  try {
      ...
      <div className="relative h-screen bg-black" data-name="video-call" data-file="components/VideoCall.js">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <div className="glass-effect rounded-lg px-6 py-4 max-w-2xl mx-auto flex items-center justify-center gap-3">
            ...
            <button
              onClick={() => {
                const roomName = `call-backup-${Date.now()}`;
                if (onJitsiFallback) {
                  onJitsiFallback(roomName);
                }
              }}
              className="w-12 h-12 rounded-full bg-yellow-600 hover:bg-yellow-700 flex items-center justify-center transition-all"
